...
class Database:
    ...
    def get_article(self, identifier):
        ...
        article= cursor.execute("select titre, identifiant, auteur, date_publication, paragraphe from article where identifiant = ?",(identifier,)).fetchone()
        if article is None:
            return None
        else:
    ...
    def get_articles(self):
        cursor = self.get_connection().cursor()
        articles=cursor.execute("select titre, identifiant, auteur, date_publication, paragraphe from article").fetchall()
    ...
    def update(self,identifier,title,parag):
        ...
        connection = self.get_connection()
        cursor= connection.cursor()
        cursor.execute(" update article set titre = ? , paragraphe = ? where identifiant = ? ",(title, parag, identifier,))
        connection.commit()
